...
import { addScore, DifficultyType, getDifficulty, getHandicapID, getHighScore, getScore, incrementMoves, resetMoves, resetScore, setDifficulty, setHighScore, setRandomHandicapID } from "./player-data.mjs";
...
export const canStackOnElem = (card: Card, elem: HTMLElement): boolean => {
    ...
    if (getHandicapID() === 12 && card.getMovingStackChlidCount() > 4) return false;
...
const swapCardElements = (cardA: Card, cardB: Card) => {
...
let elapsedSec = 0;
let lastHandicapTS: number = null; // The timestamp of the last update for handicap #2
let randomHandicap20Delay: number;
